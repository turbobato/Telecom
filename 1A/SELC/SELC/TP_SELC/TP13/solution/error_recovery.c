...
int main(int argc, char *argv[]) {
	...
	/*  Associate a signal handler to SIGSEGV and SIGBUS
  ...
  * from here in case of error */
   ...
   * usage condition of this program" */
	 if(sig_set_jmp_ret !=0)
	 	printf("An error just occured, make sure you respect the usage condition of this program (ERROR_CODE %ld)\n", sig_set_jmp_ret);
...
void  on_memory_access_error (int Num_Sig ){
	...
	// Jump to the recovery point now
