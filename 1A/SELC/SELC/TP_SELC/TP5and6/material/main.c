...
void read_file_content(Student_t * array, FILE * file){
  ...
  int i = 0;
  int ret_lec = fscanf(file, "%s %s %d", array[i].lastname,
                 array[i].firstname, &array[i].group);
  while (ret_lec != EOF) {
    i++;
    number_of_students++;
    ret_lec = fscanf(file, "%s %s %d", array[i].lastname,
                 array[i].firstname, &array[i].group);
  }
  return;
...
void display_linked_list(Link_t * list){
  Link_t *Ptr= list ;
  while (Ptr !=NULL){
    printf("%s %s %d \n", (*Ptr).student.firstname, (*Ptr).student.lastname, (*Ptr).student.group);
    Ptr = (*Ptr).next;
    }
} 
...
Link_t * search (Link_t * beginning, char * name_to_search)
{
  ...
  while (((found_link!=NULL)&&(strcmp((*found_link).student.lastname, name_to_search)!=0))){
...
Link_t * insert (Link_t * beginning, Link_t * new_link)
{
  ...
  if(beginning==NULL){
    list_after_insertion=new_link;
  }
  else if (strcmp( (*beginning).student.lastname,(*new_link).student.lastname)<=0) {
    list_after_insertion=new_link;
    new_link->next = beginning;
  }
  else {
    list_after_insertion=insert((*beginning).next, new_link);
    beginning->next=list_after_insertion;
...
Link_t * sort (Link_t * beginning)
{
  ...
  if (beginning==NULL){
    ...
    sorted_list=sort((*beginning).next);
    sorted_list=insert(sorted_list,beginning);
  }
