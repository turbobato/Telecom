...
public class Dictionnaire extends Observable {
	...
	public int chercherIndiceVariableSortante(int jE)  {
		...
		for (int i=0; i<=nbVarBase;i++){
			if (D[i][jE]>=0) {
				...
				contrainte = -D[i][0]/D[i][jE];
				// au début min vaut 0, il faut l'initialiser à la première contrainte valide rencontrée
				if (min ==0. || contrainte < min) {
					min = contrainte;
					sortante = i;
				}
			}
	...
	public void pivoter(int iS, int jE) {
		int numero_entrant = tabVarHorsBase[jE];
		int numero_sortant = tabVarBase[iS];
		double coeff_jE = D[iS][jE];
		// on pivote sur la ligne iS
		for (int j=0; j<=nbVarHorsBase; j++) {
			if (j==jE) {
				D[iS][j] = 1./coeff_jE;
			}
			else {
				D[iS][j] = -D[iS][j]/coeff_jE;
			}
		}
		// on pivote sur les autres lignes 
		for (int i=0; i<=nbVarBase; i++) {
			if (i==iS) {
				continue;
			}
			else {
				// le coefficient devant jE
				coeff_jE = D[i][jE];
				for (int j=0; j<=nbVarHorsBase; j++) {
					if (j == jE) {
						D[i][j]=coeff_jE * D[iS][jE];
					}
					else {
						D[i][j] = D[i][j] + coeff_jE * D[iS][j];
					}
				}
			}
		}
		// on échange les numéros entrants et sortant
		tabVarBase[iS]=numero_entrant;
		tabVarHorsBase[jE]=numero_sortant;
	}
	...
	public int chercherIndiceVariableEntranteAvantageuse() {	
		...
		double coeff;
		double contrainte;
		double augmentation_max= 0.;
		for (int j=1; j<= nbVarHorsBase; j++) {
			if (D[0][j] > 0.) {
				// on va boucler pour déterminer l'augmentation maximale
				coeff = D[0][j];
				double min=0.;
				boolean borne = false;
				for (int i=0; i<=nbVarBase;i++){
					if (D[i][j]>=0) {
						...
						borne = true; // on a trouvé une contrainte
						contrainte = -D[i][0]/D[i][j];
						// au début min vaut 0, il faut l'initialiser à la première contrainte valide rencontrée
						if (min ==0. || contrainte < min) {
							min = contrainte;
						}
				...
				else if (coeff * min > augmentation_max) {
					augmentation_max = coeff * min;
					indice = j;
				}
			}
		}
	...
	public int chercherIndiceVariableEntrantePlusPetitNumero(){		
		int numero=nbVarBase+nbVarHorsBase+1;
		int indice = 0;
		for (int j=1; j<= nbVarHorsBase; j++) {
			if (D[0][j] > 0 && tabVarHorsBase[j]<numero) {
				numero = tabVarHorsBase[j];
				indice = j;
			}
		}
		return indice;
	...
	public int chercherIndiceVariableSortantePlusPetitNumero(int jE) {		
		...
		for (int i=0; i<=nbVarBase;i++){
			if (D[i][jE]>=0) {
				...
				else if (contrainte == min && tabVarBase[i]<numero) {
					min = contrainte;
					numero = tabVarBase[i];
					sortante = i;
				}
